...
def test_Work_flow(test_Setup):
 ...
 search_Product = wait.until(
     EC.visibility_of_element_located((By.XPATH, '//input[@placeholder="Search for Products, Brands and More"]')))
 ...
 Scroll_Into_Product = wait.until(
     EC.visibility_of_element_located((By.XPATH, '//div[text()="vivo T4 5G (Phantom Grey, 128 GB)"]')))
 ...
 Click_Into_Product = wait.until(
     EC.visibility_of_element_located((By.XPATH, '//div[text()="vivo T4 Pro 5G (Blaze Gold, 256 GB)"]')))
